...
* Standardisation
  ...
  ![x_{stand}=\frac{x-mean(x)}{sd^*(x)}](https://latex.codecogs.com/svg.image?x_{stand}=\frac{x-mean(x)}{sd^*(x)})

  sd = standard deviation

* Normalisation

  ![x_{norm}=\frac{x-min(x)}{max(x)-min(x)}](https://latex.codecogs.com/svg.image?x_{norm}=\frac{x-min(x)}{max(x)-min(x)})
